...
function parseSetCookie(arr) {
  ...
  for (const line of (Array.isArray(arr) ? arr : [arr])) {
    ...
    if (eq < 0) continue
    out[main.slice(0, eq).trim()] = main.slice(eq + 1).trim()
  }
  return out
}
 ...
 */
export async function pollLogin(ptLoginSig, qrsig, conf) {
  const u1 = encodeURIComponent(conf.s_url)
  const url = `https://ssl.ptlogin2.qq.com/ptqrlogin?u1=${u1}&ptredirect=0&h=1&t=1&g=1&from_ui=1&ptlang=2052&action=${Math.random()}&js_ver=23111510&js_type=1&login_sig=&pt_uistyle=40&aid=${conf.appid}&daid=${conf.daid}&o1vId=&pt_js_version=v1.48.1`
  const res = await get(url, {
    headers: { ...COMMON_HEADERS, Cookie: `qrsig=${qrsig}` },
  })
  const text = res.body || ''
